...
class valorant(commands.Cog):
    ...
    async def store(self, ctx, username: Option(str, "Nhập tên đăng nhập", required=False), password: Option(str, "Nhập mật khẩu", required=False)):
    ...
    async def login(self, ctx, username: Option(str, "Nhập username"), password: Option(str, "Nhập mật khẩu")):        
        create_json('users', {})
        ...
        if login['auth'] == 'response':
            ...
            embed = discord.Embed(color=0xfd4554, description='Đăng nhập thành công tài khoản **{}**!'.format(data[str(ctx.author.id)]['IGN']))
            ...
            raise commands.UserInputError('Tài khoản hoặc mật khẩu sai!')
    ...
    async def logout(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(description='Bạn đã đăng xuất', color=0xfd4554)
            return await ctx.respond(embed=embed, ephemeral=True)
        except KeyError:
            raise commands.UserInputError("Không thể đăng xuất")
        except Exception:
            raise commands.UserInputError("Không thể đăng xuất")
            
    @slash_command(description="Set an notify for when a particular skin is in your store")
    async def notify(self, ctx, skin: Option(str, "Tên skin bạn muốn nhận thông báo")):
        await ctx.defer()
        # get_user
